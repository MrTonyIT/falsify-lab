...
const reasons = Object.freeze({
  kill: "Valid candidate exposes a correctness bug.",
  survived: "Valid candidate; target remains correct.",
  invalid:
    "Validator rejected this candidate. Check all stated format, value, aggregate and structural constraints.",
  gen_failed: "Generator could not produce usable input.",
  unusable: "Reference oracle could not reliably evaluate this candidate.",
  inconclusive: "Target execution was inconclusive; this is not a kill.",
});
export function feedbackFor(evaluation, attempt) {
  assert(
    Object.values(Verdict).includes(evaluation.verdict),
    "Unknown verdict",
  );
  return Object.freeze({
    attempt,
    verdict: evaluation.verdict,
    input: Buffer.from(evaluation.data ?? "")
      .toString("utf8")
      .slice(0, 500),
  });
}
export function build_falsify_prompt(
  problem,
  target,
  history = [],
  { knownWrong = true } = {},
  ...
  const p = publicProblem(problem),
    t = devTarget(target);
  const visibleHistory = history.map((h) => ({
    attempt: h.attempt,
    verdict: h.verdict,
    input: String(h.input ?? "").slice(0, 500),
    reason: reasons[h.verdict],
  }));
  const language = languageFor(target.language);
  const languageName = language.id === "python" ? "Python" : language.name;
  const targetContract = contract.replace(
    "Python code",
    languageName + " code",
  );
  const instructions = knownWrong
    ? targetContract
    : targetContract.replace(
        "code KNOWN TO BE INCORRECT",
        "code whose correctness is UNKNOWN",
      );
  return (
    instructions +
    "\n[PROBLEM]\n" +
...
export function parseResponse(response) {
  if (typeof response !== "string" || !response.trim())
    return { script: null, error: "empty_response" };
  const blocks = [...response.matchAll(/```python[ \t]*\r?\n([\s\S]*?)```/g)]
    .map((m) => m[1].trim())
    .filter(Boolean);
  return blocks.length
    ? { script: blocks.at(-1), error: null }
    : { script: null, error: "missing_python_block" };
}
