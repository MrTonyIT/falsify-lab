...
export function validate(data) {
  try {
    const lines = strictLines(data);
    const [t] = integers(lines[0], 1, 1, 5);
    if (lines.length !== 1 + 2 * t) return { ok: false };
    let total = 0;
    for (let i = 0; i < t; i++) {
      const [n] = integers(lines[1 + 2 * i], 1, 1, 10);
      integers(lines[2 + 2 * i], n, -10, 10);
      total += n;
    }
...
export function smokeProblem() {
  return {
    oracleKind: "synthetic-fixture",
    id: "synthetic-sum",
    statement: "For each test case, print the sum of its array.",
    constraints:
      "1 <= t <= 5; 1 <= n <= 10; -10 <= a[i] <= 10; sum(n) <= 20. First line t; each case has a line n and a line of exactly n integers.",
    division: "A",
    publishedAt: null,
    semanticSizeDefinition: "sum(n) across cases",
    validator: validate,
    validatorCases,
    validatorHash: sha256(String(validate)),
    randomGenerator,
    randomHash: sha256(String(randomGenerator)),
    checker: tokenChecker,
    checkerName: "tokens",
    samples: [{ input: "1\n2\n1 2\n", output: "3\n" }],
    references: references.map((code, i) => ({
      id: "ref" + i,
    ...
    dev: [
      sub(
        "dev1",
        "dev",
        "t=int(input())\nfor _ in range(t):\n n=int(input()); print(sum(abs(x) for x in map(int,input().split())))",
      ),
    ],
    heldOut: [
      sub(
