...
export async function smoke(directory) {
  ...
  const sandbox = new MockSandbox((code, input, { role }) => {
    ...
    const value =
      role === "reference"
        ...
        : code.includes("abs")
          ? values.reduce((a, b) => a + Math.abs(b), 0)
          : values.reduce((a, b) => a + Math.max(0, b), 0);
  ...
  const evaluator = new Evaluator(sandbox),
    ...
    metadata = metadataFor([p], config, "smoke");
  const log = await initializeRun(directory, metadata);
  await runBaselines({
    problems: [p],
    frozen: freezeBaseline([p]),
    evaluator,
    log,
    metadata,
  });
  ...
  assert(
    result.official_status === "NOT RUN",
    "Fixture must never produce official findings",
  );
  return result;
}
